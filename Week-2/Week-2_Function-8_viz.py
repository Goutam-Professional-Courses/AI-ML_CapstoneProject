...
# kernel: Kernel = Matern(length_scale=1.0, length_scale_bounds="fixed")
...
# -----------------------------
grid_size = 500
x_grid = np.linspace(0, 1, 8 * grid_size).reshape(-1, 8)
...
confid_intvl = 0.20
grph.plot2D(
    weekNbr, funcNbr, x_grid, y_pred_means, y_pred_sigmas, confid_intvl, X_inputs, Y_outputs, output_lower_limit=0, output_upper_limit=10, output_step=1
)
plt.show()
