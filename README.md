...
-   Week-4: After 3 weeks I found that a large amount of <b>Python</b> code was almost same for any given function across weeks. I began to refactor my codebase
    to move the common code into reusable modules and then invoke their functionality from the week specific Jupyter notebooks. The emphasis of the search
    strategy started to move towards exploitation from pure exploration. I found it useful to tweak the `length_scale` hyper-parameter of GP kernels, as this
    help us to control the correlation between data-points in a more granular manner and observe the variations in the output more closely.

-   Week-5: After another week, I found that there is still a significant amount of code that is largely same across the functions, regardless of dimensions.
    This resulted in a 2nd round of refactoring, where more of Python source code was extracted from the function specific Jupyter notebooks and moved into
    reusable modules. The 2 rounds of refactoring eliminated a lot of code duplication and established codebase uniformity in subsequent weeks.

-   Week-6: Not all features have the similar degree of influence on the 8 unknown functions. This fact is more pronounced in the higher-dimensional functions
    and can be a crucial piece of information to narrow down the global optimum points much more quickly than performing extensive search throughout the feature
    space. In accordance with this thought, I introduced a 2nd ancillary Python module (named in the pattern: <b>Week-<idx>\_Function-<idx>\_FeatureImp.py</b>)
    that would depict the degree of influence of the input features of each function as horizontal bar diagrams using a technique called <b>Permutation
    importance</b>.

-   Week-7: While I was mostly using the <code>RBF</code> (Radial basis function) in the earlier weeks, I found the <code>Matern</code> kernel to be more
    effective later on since the smoothness of the predicted curve can be adjusted through the `nu` hyper-parameter. I tweaked the `length_scale`
    hyper-parameter of GP kernels and set `length_scale_bounds` to a range of values, rather than keeping it <b>fixed</b>. That means, the `length_scale` can be
    varied during hyper-parameter tuning, which will require more computational power, but likely to discover more favourable outputs.
