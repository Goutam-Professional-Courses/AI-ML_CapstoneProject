...
from sklearn.gaussian_process.kernels import DotProduct, Kernel, RBF, ConstantKernel as C, Matern, WhiteKernel
...
# -----------------------------
grid_size = 500
x_grid = np.linspace(0, 1, 3 * grid_size).reshape(-1, 3)
...
grph.plot2D(
    ...
    X_inputs,
    Y_outputs,
    output_lower_limit=-0.8,
    output_upper_limit=-0.001,
    output_step=0.05,
)
plt.show()
