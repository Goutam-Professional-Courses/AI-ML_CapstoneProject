...
from sklearn.gaussian_process.kernels import Kernel, RBF, ConstantKernel as C, Matern
...
kernel: Kernel = Matern(length_scale=1.0, length_scale_bounds="fixed", nu=1.5)
...
# -----------------------------
grid_size = 500
x_grid = np.linspace(0, 1, 4 * grid_size).reshape(-1, 4)
...
# -----------------------------
confid_intvl: float = 0.20
grph.plot2D(
    weekNbr, funcNbr, x_grid, y_pred_means, y_pred_sigmas, confid_intvl, X_inputs, Y_outputs, output_lower_limit=-50, output_upper_limit=0, output_step=5
)
plt.show()
