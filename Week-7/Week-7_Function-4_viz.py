...
kernel: Kernel = Matern(length_scale=0.1, length_scale_bounds=(1e-3, 1e3), nu=1.5)
model = GaussianProcessRegressor(kernel=kernel, n_restarts_optimizer=9, normalize_y=False)
grid_size = 500
...
grph.plotFunction(
    weekNbr, funcNbr, x_grid, y_pred_means, y_pred_sigmas, confid_intvl, X_inputs, Y_outputs, output_lower_limit=-40, output_upper_limit=10, output_step=5
)
plt.show()
